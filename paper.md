...
The FANTOM6 consortium performed knock-down experiments with lncRNAs antisense oligos, followed by the expression profiling using the CAGE protocol (KD-CAGE). The CAGE protocol can identify active transcription start sites in the genome with their expression levels. The original purpose of the lncRNA KD-CAGE is for predicting the function of each lncRNA by observing the changes of expression levels. In the pilot phase, they performed ~300 KD-CAGE assays to the human dermal fibroblast cells. The result data is publicly available in the public repository and their web site (https://fantom.gsc.riken.jp/6/).

Although the original aims of the lncRNA KD-CAGE is to elucidate the function of lncRNAs, the expression profile data can potentially be reused for other analysis. For example, if the genes changed by the knock-down of specific lncRNAs are matched with the list of differentially expressed genes in a biological phenotype/stimulus, we can infer that the phenotype or stimulus can be related to the lncRNA.

To achieve such type of the analysis, we discussed the actual analysis method. Through the discussion, we concluded that an enrichment analysis is easier for this purpose. A user input is a list of interesting genes. The list is compared with the lists of genes obtained by FANTOM6 lncRNA KD-CAGE, and adjusted P-values of enrichments are calculated with Fisher's exact test or similar one. To achieve the method, we need to the following as a first step. First, we retrieve the list of changed genes from the FANTOM6 lncRNA KD-CAGE expression profiles. Next, we implement a web-based system for performing the analysis, which is useful for wet researches who do not have enough computation skills.

In addition to the above function, the system should support or consider the following extensions. The first is extensibility of the FANTOM6 KD data. The FANTOM6 project is ongoing, and more data will be released later using different lncRNAs and cell types. After the continuous releases of the datasets, the system should be able to easily process and load the new datasets. The second is expansion to more public KD datasets. The analysis is not limited to the FANTOM6 KD-CAGE data. Thus, if we can collect more KD data from public repositories, we can make the system to provide more rich results. These are future plans after the prototype system.
